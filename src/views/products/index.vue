  ...
  >
    <template #headerCell="{ column }">
      <template v-if="column.key === 'action'">
        操作
        <router-link :to="$_append('add')"><a-button size="small">添加</a-button></router-link>
      </template>
    </template>
    <template #bodyCell="{ column, record }">
      <template v-if="column.key === '#'">
        <div class="flex">
          ...
          <div class="flex-auto flex flex-col justify-between">
            <div><span class="text-gray-300">{{ record.id }}:</span>
              {{ record.name }}
            </div>
            <div>
              ¥{{ record.price }}
      ...
      <template v-else-if="column.key === 'action'">
        <router-link :to="$_append(record.id)"><a-button size="small">编辑</a-button></router-link>
        <a-button @click="onDelete(record.id)" type="link" size="small">删除</a-button>
      </template>
    </template>
  </a-table>
</a-card>
...
import {reqPagination, reqDelete} from '@/utils/useRequest';
...
const onDelete = async (id) => {
  $msg.confirm({
    ...
    onOk() {
      ...
      watch(loading, () => {
        if (error.value) return $msg.error('删除失败');
        run();
      });
    },
  });
